...
export function Home() {
    ...
    if (isLoading && !spotlightMovie) {
        return (<div className="flex min-h-screen items-center justify-center px-4 pt-16">
    ...
    return (<div className="min-h-screen pb-16 pt-16">
            ...
            <div className="relative z-10 grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(300px,360px)] lg:items-center lg:gap-8">
              <div className="max-w-2xl xl:max-w-[42rem]">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  {spotlightStats.map((stat) => (<div key={stat.label} className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 backdrop-blur-sm">
              ...
              <div className="mx-auto w-full max-w-[20rem] lg:mx-0 lg:justify-self-end">
                <div className="rounded-[1.6rem] border border-white/10 bg-black/25 p-4 backdrop-blur-sm">
                  <PosterImage src={spotlightMovie.poster} title={spotlightMovie.title} className="aspect-[2/3] max-h-[30rem] w-full rounded-[1.35rem] object-cover"/>
                  <div className="mt-4">
                    <VerdictBadge verdict={spotlightMovie.verdict} score={spotlightMovie.score} size="lg"/>
                  </div>
                  <div className="mt-4 grid gap-2.5">
                    {spotlightStats.slice(0, 2).map((stat) => (<div key={stat.label} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3">
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <div className="section-panel p-6 sm:p-8">
            ...
            <div className="mt-6 grid items-start gap-5 xl:grid-cols-[0.95fr_1.05fr]">
              {latestMovies[0] && (<div className="self-start rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-4">
                  <MovieCard movie={latestMovies[0]} variant="hero" onClick={() => navigate(`/review/${latestMovies[0].id}`)}/>
                </div>)}
              <div className="grid items-start gap-5 sm:grid-cols-2">
                {latestMovies.slice(1, 5).map((movie) => (<MovieCard key={movie.id} movie={movie} variant="compact" onClick={() => navigate(`/review/${movie.id}`)}/>))}
              </div>
            </div>
          </div>
          ...
          <div className="section-panel p-6 sm:p-8">
            ...
            <div className="mt-6 space-y-3">
              {trendingMovies.slice(0, 5).map((movie, index) => (<button key={movie.id} onClick={() => navigate(`/review/${movie.id}`)} className="group flex w-full items-center gap-4 rounded-[1.5rem] border border-white/10 bg-white/[0.03] px-4 py-4 text-left transition-all hover:border-[#d26d47]/40 hover:bg-white/[0.05]">
      ...
      <section className="section-shell mt-8">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1fr_1fr]">
          <div className="section-panel p-6 sm:p-8">
            ...
            <div className="mt-6 grid grid-cols-2 gap-5 md:grid-cols-4">
              {topRatedMovies.slice(0, 4).map((movie) => (<MovieCard key={movie.id} movie={movie} variant="compact" onClick={() => navigate(`/review/${movie.id}`)}/>))}
            </div>
          </div>
        ...
        ].map((item) => (<div key={item.label} className="flex items-center justify-between rounded-[1.5rem] border border-white/10 bg-white/[0.03] px-4 py-4">
