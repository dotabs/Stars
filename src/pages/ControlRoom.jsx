...
export function ControlRoom() {
    ...
    const [preferredGenres, setPreferredGenres] = useState(['Drama', 'Sci-Fi', 'Thriller']);
    // Discovery rules
    ...
    const [pinnedCountries, setPinnedCountries] = useState(['USA', 'Japan', 'UK']);
    // Layout
        ...
        switch (activeMenu) {
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {genres.map((genre) => (<button key={genre} onClick={() => toggleGenre(genre)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${preferredGenres.includes(genre)
            ...
            <div>
              ...
              <div className="space-y-6">
                {Object.entries(genreWeights).map(([genre, weight]) => (<div key={genre}>
                    <div className="flex justify-between mb-2">
                      <span className="text-sm">{genre}</span>
                      <span className="text-sm text-muted-foreground">{weight}/10</span>
                    </div>
                    <Slider value={[weight]} onValueChange={([v]) => setGenreWeights({ ...genreWeights, [genre]: v })} min={1} max={10} step={1}/>
                  </div>))}
              </div>
            </div>
          </div>);
            ...
            <div>
              ...
              <div className="flex gap-3">
                {['daily', 'twice-weekly', 'weekly'].map((freq) => (<button key={freq} onClick={() => setSpotlightFrequency(freq)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${spotlightFrequency === freq
                            ? 'bg-primary text-white'
                            : 'bg-white/5 text-muted-foreground hover:bg-white/10'}`}>
            ...
            <div>
              ...
              <div className="flex gap-3">
                {[
                        ...
                        { id: 'both', label: 'Both' }
                    ].map((source) => (<button key={source.id} onClick={() => setSpotlightSource(source.id)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${spotlightSource === source.id
                            ? 'bg-primary text-white'
                            : 'bg-white/5 text-muted-foreground hover:bg-white/10'}`}>
                    {source.label}
                  </button>))}
            ...
            <div>
              ...
              <div className="space-y-2">
                {sections.map((section, index) => (<div key={section.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <Slider value={[globeBias]} onValueChange={([v]) => setGlobeBias(v)} min={0} max={100} step={10} className="flex-1"/>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {pinnedCountries.map((country) => (<span key={country} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-primary/20 text-primary text-sm">
                    {country}
      ...
      <div className="flex min-h-[calc(100vh-64px)] flex-col lg:flex-row">
        ...
        <aside className="lg:hidden border-b border-white/[0.06] bg-card/30 px-4 py-4">
          ...
          <nav className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {menuItems.map((item) => (<button key={item.id} onClick={() => setActiveMenu(item.id)} className={`flex items-center gap-2 px-4 py-2.5 rounded-full whitespace-nowrap transition-all ${activeMenu === item.id
        ...
        <aside className="hidden lg:block lg:w-64 lg:flex-shrink-0 bg-card/30 border-r border-white/[0.06] p-4 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
          ...
          <nav className="space-y-1">
            {menuItems.map((item) => (<button key={item.id} onClick={() => setActiveMenu(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all ${activeMenu === item.id
                ? 'bg-primary/20 text-primary'
