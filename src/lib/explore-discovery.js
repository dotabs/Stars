...
function getLocalCountryFallback(countryKey) {
...
function createSeedFromCountry(countryKey) {
    let seed = 0;
...
async function fetchBucketMovies(countryKey, definition, genreMap) {
...
function buildCountryPool(countryKey, movies, totalPoolResults, source) {
    ...
    const averageScore = sortedMovies.length === 0
        ? 0
...
function bucketizeMovies(movies) {
    const currentYear = new Date().getFullYear();
    return {
        topPicks: [...movies].sort((left, right) => (right.popularity ?? 0) - (left.popularity ?? 0) ||
            (right.reviewCount ?? 0) - (left.reviewCount ?? 0) ||
            right.score - left.score),
        newestPicks: [...movies].sort((left, right) => (right.releaseDate ?? `${right.year}-01-01`).localeCompare(left.releaseDate ?? `${left.year}-01-01`) ||
            right.score - left.score),
        hiddenGems: [...movies]
...
export async function fetchExploreCountryPool(countryKey, options) {
...
export function prefetchExploreCountryPool(countryKeys) {
    void Promise.all(countryKeys
