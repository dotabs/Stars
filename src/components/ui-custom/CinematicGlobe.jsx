...
import { exploreCatalogByGlobeName } from '@/data/explore';
const OCEAN = '#0B1220';
...
const emptyCountryCache = new Map();
function getEmptyCountryStat(name) {
...
function toExploreCountryStat(country) {
    if ('filmCount' in country)
        return country;
    return {
        key: country.key,
        label: country.label,
...
function shortestRotation(start, target) {
...
export function CinematicGlobe({ countries, selectedCountryKey, pinnedCountryKeys, spinToken, onCountrySelect, onSpinStateChange, className, }) {
    const wrapperRef = useRef(null);
    const svgRef = useRef(null);
    const animationFrameRef = useRef(null);
    const autoRotateFrameRef = useRef(null);
    const rotationRef = useRef([18, -18, 0]);
    ...
    const [rotation, setRotation] = useState([18, -18, 0]);
    ...
    const countriesGeo = useMemo(() => feature(worldAtlas, worldAtlas.objects
        .countries), []);
    const countryFeatures = countriesGeo.features;
    const featureByName = useMemo(() => new Map(countryFeatures
        .map((country) => [country.properties?.name ?? '', country])
        .filter(([name]) => Boolean(name))), [countryFeatures]);
    const selectedCountry = useMemo(() => countries.find((country) => country.key === selectedCountryKey) ?? countries[0], [countries, selectedCountryKey]);
    const countryByGlobeName = useMemo(() => new Map(countries.map((country) => [country.globeName, country])), [countries]);
    const projection = useMemo(() => d3
        ...
        .rotate(rotation)
        .clipAngle(90)
        .precision(0.1), [globeScale, rotation, size]);
    const path = useMemo(() => d3.geoPath(projection), [projection]);
    const sphere = useMemo(() => ({ type: 'Sphere' }), []);
    const graticule = useMemo(() => d3.geoGraticule10(), []);
    const centerCoordinates = useMemo(() => [-rotation[0], -rotation[1]], [rotation]);
    const markerRadiusScale = useMemo(() => d3
        .scaleSqrt()
        .domain([0, d3.max(countries, (country) => country.filmCount) ?? 1])
        .range([3, 13]), [countries]);
    const setRotationState = useCallback((nextRotation) => {
        const normalized = [
    ...
    const setTooltipForCountry = useCallback((countryName, clientX, clientY) => {
        const bounds = wrapperRef.current?.getBoundingClientRect();
        const countryCandidate = countryByGlobeName.get(countryName) ??
    ...
    return (<div ref={wrapperRef} className={className}>
      ...
      <div className="absolute inset-[14%] rounded-full opacity-40" style={{
            boxShadow: `0 0 120px ${SOFT_GLOW}, inset 0 0 60px rgba(255,255,255,0.03)`,
        }}/>

      <svg ref={svgRef} viewBox={`0 0 ${size} ${size}`} className="relative z-10 h-full w-full cursor-grab active:cursor-grabbing select-none" role="img" aria-label="Interactive cinematic globe">
        <defs>
        ...
        <path d={path(graticule) ?? undefined} fill="none" stroke={GRID_LINE} strokeWidth={0.8}/>

        {countryFeatures.map((country) => {
            const countryName = country.properties?.name ?? '';
            const statCandidate = countryByGlobeName.get(countryName) ?? exploreCatalogByGlobeName.get(countryName);
            ...
            if (stat?.explored)
                fill = 'rgba(243,200,106,0.18)';
            if (isPinned)
                fill = 'rgba(255,255,255,0.12)';
            if (isHovered)
                fill = 'rgba(255,255,255,0.18)';
            if (isSelected)
                fill = 'rgba(255,59,59,0.42)';
            return (<path key={countryName || JSON.stringify(country.geometry)} d={path(country) ?? undefined} fill={fill} stroke={isSelected ? 'rgba(255,255,255,0.45)' : COUNTRY_BORDER} strokeWidth={isSelected ? 1.15 : 0.7} style={{
        ...
        {markerData.map(({ country, projected, radius }) => {
            ...
            return (<g key={country.key} transform={`translate(${projected[0]}, ${projected[1]})`} className="cursor-pointer" onMouseEnter={(event) => {
              ...
              <circle r={radius + 5} fill={country.explored ? 'rgba(243,200,106,0.16)' : SOFT_GLOW} filter="url(#stars-marker-glow)"/>
              <circle r={radius + (isSelected ? 2 : 0)} fill={isSelected ? ACCENT_RED : country.explored ? EXPLORED_GOLD : '#FFD2D2'} stroke="rgba(255,255,255,0.5)" strokeWidth={isSelected ? 1.4 : 0.9}/>
              <circle r={Math.max(radius - 2, 1.5)} fill={isSelected ? '#FFE7E7' : OCEAN} opacity={0.95}/>
            </g>);
        })}

        <path d={path(sphere) ?? undefined} fill="none" stroke="rgba(255,255,255,0.14)" strokeWidth={1}/>
      </svg>

      {tooltip && (<div className="pointer-events-none absolute z-20 hidden w-64 rounded-2xl border border-white/10 bg-[#0d121d]/95 p-4 shadow-2xl backdrop-blur-xl md:block" style={{
