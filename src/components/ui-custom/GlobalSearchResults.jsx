...
function buildSearchFallbackMovie(result) {
    ...
    return {
        ...
        title: result.title,
        year,
        genres: result.mediaType === 'person'
...
function buildPersonMovie(details) {
...
export function SearchResultCard({ result, onOpen, isInWatchlist = false, isLiked = false, onToggleWatchlist, onToggleLike, }) {
    const baseState = useMemo(() => buildBaseResolvedState(result), [result]);
    const cacheKey = `${result.mediaType}-${result.id}`;
    const [resolvedStateMap, setResolvedStateMap] = useState({});
    useEffect(() => {
        let cancelled = false;
        ...
        const readOrCreatePromise = () => {
            ...
            const nextPromise = result.mediaType === 'movie'
        ...
        void readOrCreatePromise().then((nextState) => {
            ...
            if (!cancelled) {
                setResolvedStateMap((currentState) => {
                    if (currentState[cacheKey] === nextState)
                        return currentState;
                    return {
                        ...currentState,
                        [cacheKey]: nextState,
    ...
    const display = useMemo(() => {
        ...
        if (result.mediaType === 'tv') {
            return {
                subtitle,
                badges: [
                    { label: movie.score > 0 ? movie.score.toFixed(1) : 'TV', icon: Star, tone: 'primary' },
                    { label: `${Math.max(movie.reviewCount ?? 0, 1)} eps`, icon: Tv, tone: 'secondary' },
                ],
    ...
    return (<MovieCard movie={movie} variant="compact" onClick={() => onOpen?.(href)} onToggleWatchlist={result.mediaType === 'movie' ? onToggleWatchlist : undefined} onToggleLike={result.mediaType === 'movie' ? onToggleLike : undefined} isInWatchlist={result.mediaType === 'movie' ? isInWatchlist : false} isLiked={result.mediaType === 'movie' ? isLiked : false} display={display}/>);
}
export function SearchTypeIcon({ mediaType, className }) {
    if (mediaType === 'movie')
...
export function SearchResultEmptyCard() {
    return (<div className="flex min-h-[24rem] flex-col items-center justify-center rounded-[1.7rem] border border-dashed border-white/10 bg-white/[0.02] px-6 text-center">
